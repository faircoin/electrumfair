...
for i in ./locale/*; do
    dir=$WINEPREFIX/drive_c/electrumfair/electrumfair/$i/LC_MESSAGES
    mkdir -p $dir
    msgfmt --output-file=$dir/electrumfair.mo $i/electrumfair.po || true
done
...
# $VERSION could be passed to the electrumfair.nsi script, but this would require some rewriting in the script itself.
wine "$WINEPREFIX/drive_c/Program Files (x86)/NSIS/makensis.exe" /DPRODUCT_VERSION=$VERSION electrumfair.nsi
...
sha256sum dist/ElectrumFair*.exe
