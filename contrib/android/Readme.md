...
2. Build image
    ...
    ```
    $ sudo docker build -t electrumfair-android-builder-img contrib/android
    ```
...
```
$ adb -d install -r bin/ElectrumFair-*-arm64-v8a-debug.apk
$ adb -d install -r bin/ElectrumFair-*-armeabi-v7a-debug.apk
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrumfair.electrumfair | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrumfair -g kivy`
...
```
$ adb shell
$ run-as org.electrumfair.electrumfair ls /data/data/org.electrumfair.electrumfair/files/data
$ run-as org.electrumfair.electrumfair cp /data/data/org.electrumfair.electrumfair/files/data/wallets/my_wallet /sdcard/some_path/my_wallet
```
